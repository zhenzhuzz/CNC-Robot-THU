function simple_plc_tcp()
    ...
    %% 创建PLC定时器（100Hz），用于发送命令和接收数据
    plcTimer = timer('ExecutionMode', 'fixedSpacing', 'Period', 1, 'TimerFcn', @plcTimerFcn);
    
    %% 创建加速度计定时器（100Hz或根据需要调整），用于处理UDP数据
