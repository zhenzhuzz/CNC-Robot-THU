function simple_plc_tcp()
    ...
    isRecording = false;
    % 使用 cell 数组预先分配1000行3列：列依次为 [RelativeTime, rawAcc (原始加速度数据), spinSpeed]
    dataLog = cell(1000, 3);
    dataCount = 0;  % 记录有效数据行数
    % 记录开始时刻（使用 tic/toc 高精度计时），初始为空
    recordStartTime = [];
    ...
    %% 创建加速度计定时器（用于处理UDP数据）
    % 注意：BusyMode 设置为 'drop'，避免回调堆积
    accTimer = timer('ExecutionMode', 'fixedSpacing', 'Period', 0.001, 'BusyMode','drop','TimerFcn', @accTimerFcn);
    ...
    function accTimerFcn(~, ~)
        if isAccConnected && ~isempty(udpObj) && udpObj.NumDatagramsAvailable > 0
            ...
            for k = 1:length(datagrams)
                ...
                if numel(packet) >= 10
                    % 这里暂时不调用解析函数，也不在命令行 disp 输出
                    % [acc_x, acc_y, acc_z] = parseAccPacket(packet); % 注释掉解析调用
                    % 直接记录原始数据包（原样保存）
                    if isRecording
                        t = toc(recordStartTime);
                        if isPLCConnected
    ...
    %% 数据记录：将一行数据存入工作区预分配的 cell 数组
    function logDataRow(t, rawAcc, spin)
        % newRow 为 cell 类型：[时间, 原始加速度数据, spinSpeed]
        newRow = {t, rawAcc, spin};
        dataCount = dataCount + 1;
        if dataCount > size(dataLog, 1)
            % 超出预分配大小则扩展 cell 数组（每次增加 1000 行）
            dataLog = [dataLog; cell(1000, 3)];
        end
    ...
    function recordButtonCallback(src, ~)
        ...
        if ~isRecording
            % 开始记录：初始化预分配 cell 数组、计数器，并用 tic 初始化高精度计时
            dataLog = cell(1000, 3);
            dataCount = 0;
            recordStartTime = tic;
            isRecording = true;
