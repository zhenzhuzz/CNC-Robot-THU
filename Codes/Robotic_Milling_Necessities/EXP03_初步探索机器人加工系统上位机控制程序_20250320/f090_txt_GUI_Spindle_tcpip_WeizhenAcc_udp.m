function simple_plc_tcp()
    ...
    plcTimer = timer('ExecutionMode', 'fixedSpacing', 'Period', 0.001, 'TimerFcn', @plcTimerFcn);
    
    %% 创建加速度计定时器（用于处理UDP数据）
    accTimer = timer('ExecutionMode', 'fixedSpacing', 'Period', 0.001, 'TimerFcn', @accTimerFcn);
