function simple_plc_tcp()
    ...
    % 新增：用于加速度数据记录的缓存区及计数器
    dataBuffer = [];           % 用于存储原始UDP数据包（结构体数组）
    accTimerTriggerCount = 0;  % 记录accTimer触发次数
    maxBufferLength = 13200;   % 预留2秒数据缓存（约2秒×6600包/秒）
    ...
    function accTimerFcn(~, ~)
        if isAccConnected && ~isempty(udpObj) && udpObj.NumDatagramsAvailable > 0
            if isRecording
                % 每次触发先将原始数据存入缓存区
                rawDatagrams = read(udpObj, udpObj.NumDatagramsAvailable, "uint8");
                rawDatagrams = rawDatagrams(:);  % 转换为列向量，确保尺寸一致
                
                if isempty(dataBuffer)
                    dataBuffer = rawDatagrams;
                else
                    dataBuffer = [dataBuffer; rawDatagrams];
                end
                accTimerTriggerCount = accTimerTriggerCount + 1;
                % 每10次触发或缓存区超过预设2秒数据时，解析并写入文件
                if mod(accTimerTriggerCount, 10) == 0 || length(dataBuffer) > maxBufferLength
                    processBufferedData();
                end
            else
                % 非记录状态下，直接处理数据（用于显示等）
        ...
        for k = 1:length(datagrams)
            ...
            if numel(packet) >= 10
                ...
                disp(['加速度计数据 -> acc_x: ', num2str(acc_x), ...
                      ', acc_y: ', num2str(acc_y), ...
                      ', acc_z: ', num2str(acc_z)]);
                % 若正在记录且加速度上电，则记录数据行，同时若PLC上电填充最新转速
                if isRecording
                    t = datetime('now');
                    if isPLCConnected
                        spin = lastSpinSpeed;
    ...
    %% 新增：批量处理缓存数据函数，每10次触发或缓存满2秒数据时调用
    function processBufferedData()
        for k = 1:length(dataBuffer)
            packet = dataBuffer(k).Data;
            if numel(packet) >= 10
                [acc_x, acc_y, acc_z] = parseAccPacket(packet);
                t = datetime('now');
                if isPLCConnected
