function simple_plc_tcp()
    ...
    % 数据记录相关变量（改为记录到工作区，不写文件）
    isRecording = false;
    % 预先分配1000行5列的矩阵：列依次为 [RelativeTime, accX, accY, accZ, spinSpeed]
    dataLog = zeros(1000, 5);
    ...
    %% 创建加速度计定时器（用于处理UDP数据）
    accTimer = timer('ExecutionMode', 'fixedSpacing', 'Period', 0.05, 'BusyMode','drop','TimerFcn', @accTimerFcn);
    ...
    %% 数据记录：将一行数据存入工作区预分配的矩阵
    function logDataRow(t, ax, ay, az, spin)
        % 使用高精度计时，记录开始时刻为0，时间单位为秒
        elapsedTime = toc(recordStartTime);
        newRow = [elapsedTime, ax, ay, az, spin];
        dataCount = dataCount + 1;
        if dataCount > size(dataLog, 1)
            % 超出预分配大小则扩展矩阵（每次增加1000行）
            dataLog(end+1:end+1000, :) = 0;
        end
    ...
    function recordButtonCallback(src, ~)
        ...
        if ~isRecording
            % 开始记录：初始化预分配矩阵、计数器，并使用 tic 初始化高精度计时
            dataLog = zeros(1000, 5);
            dataCount = 0;
            recordStartTime = tic;
            isRecording = true;
