%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% m057_accXYZ_ABBcontrol_CustomSpinSpeed_NI_Spindle_TCPIP_GUI_04.m
%
% 说明：
%   本程序实现了主轴与加速度计的联合控制及数据监测、记录功能，
%   并在GUI中增加了“自定义调速”功能及 ABB 机器人相关控制功能。
%
%   自定义调速功能说明：
...
%
%   ABB 机器人相关功能说明：
%       - 主控制界面中在“ABB机器人”区域放置上电、下电、启动（图标 ▶）、
%         停止（图标 ■）及设定速度控件。
%       - 第二行按钮区域增加“指针复位”按钮和“建立连接”按钮。
%       - “建立连接”按钮点击后，如果未建立连接则弹出单独的模态窗口（包含“刷新”、“连接”
%         和控制器列表）；如果已建立连接，则直接断开连接并切换按钮文本。
%       - 其他所有机器人相关操作均判断是否已建立连接，若未建立则提示相关信息输出到“机器人状态显示”区域。
%
% 作者：Zhen Zhu
% 日期：Every day
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

function m059_FontSize_Update_accXYZ_ABBcontrol_NI_Spindle_TCPIP_GUI_04()
    %% 初始化参数
    ...
    % 获取屏幕尺寸并设置为全屏
    screenSize = get(0, 'ScreenSize');  % 获取屏幕尺寸 [left bottom width height]
    
    % 创建GUI窗口，设置为全屏
    % fig = uifigure('Name', '主轴&加速度计&ABB机器人控制面板', 'Position', screenSize);  % 使用屏幕尺寸设置全屏
    ...
    %% 第一行：控制面板区域（分为3列）
    controlPanel = uipanel(mainLayout, 'Title', '控制面板','FontSize',40,'FontWeight','bold');
    ...
    % --- 左侧：主轴控制区域 ---
    spindlePanel = uipanel(cpLayout, 'Title', '主轴','FontSize',40,'FontWeight','bold');
    ...
    % --- 中间：加速度计控制区域 ---
    accelPanel = uipanel(cpLayout, 'Title', '加速度计','FontSize',40,'FontWeight','bold');
    ...
    % --- 右侧：ABB机器人控制区域 ---
    robotPanel = uipanel(cpLayout, 'Title', 'ABB机器人','FontSize',40,'FontWeight','bold');
    ...
    %% 第三行：数据显示区域（分为3列）
    displayPanel = uipanel(mainLayout, 'Title', '数据显示','FontSize',30);
    ...
    %% 调整字号
    % 设置所有文本框控件的字体大小
    txtSpeed.FontSize = 30;  % 设置转速文本框字体大小
    txtSamplingRate.FontSize = 30;  % 设置采样率文本框字体大小
    txtRobotSpeed.FontSize = 30;  % 设置速度文本框字体大小

    % 设置显示区域的字体大小
    txtSpindleDisplay.FontSize = 30;  % 设置为你想要的字体大小
    txtAccelDisplay.FontSize = 30;    % 设置为你想要的字体大小
    txtRobotStatusDisplay.FontSize = 30;  % 设置为你想要的字体大小

    % 调整所有文本控件的字体大小
    setFontSizeForAllTextControls(fig, 30);  % 调整为你想要的字体大小
    ...
    function abbMsg(msg)
        ...
        if isRecordingMessages
            robotMessages = [robotMessages; {msg}];  % 将消息添加到机器人日志
        end
    end
    ...
    function recordButtonCallback(src, ~)
        ...
        if ~isRecording
            ...
            if ~isempty(spindleMessages) || ~isempty(accelMessages) || ~isempty(robotMessages)
                logFolder = fullfile(pwd, 'logs');  % 当前文件夹下的 'logs' 文件夹
                if ~exist(logFolder, 'dir')
                    mkdir(logFolder);  % 如果没有 'logs' 文件夹，则创建
                end
                % 获取当前时间，并生成文件名
                ...
                try
                    ...
                    % 合并并写入三列消息
                    maxLen = max([length(spindleMessages), length(accelMessages), length(robotMessages)]);
                    ...
                    for i = 1:maxLen
                        ...
                        if i <= length(spindleMessages)
                            spindleMsg = spindleMessages{i};
                        end
                        if i <= length(accelMessages)
                            accelMsg = accelMessages{i};
                        end
                        if i <= length(robotMessages)
                            robotMsg = robotMessages{i};
                        end
                        
                        fprintf(fid, '%s\t%s\t%s\n', spindleMsg, accelMsg, robotMsg);
                    end
    ...
    function prependSpindleDisplay(msg)
        ...
        if isRecordingMessages
            % 将消息添加到日志并加一行空行
            spindleMessages = [spindleMessages; {msg}; {''}];  % 添加消息后加一行空行
        end
    end
    ...
    function prependAccelDisplay(msg)
        ...
        if isRecordingMessages
            accelMessages = [accelMessages; {msg}];  % 将消息添加到加速度计日志
        end
    end
    ...
    function setFontSizeForAllTextControls(fig, fontSize)
        % 获取GUI中所有控件
        allControls = findall(fig);
    
        % 遍历所有控件，找到所有文本控件并设置字体大小
        for i = 1:length(allControls)
            control = allControls(i);
            if isprop(control, 'Text') && ischar(control.Text)
