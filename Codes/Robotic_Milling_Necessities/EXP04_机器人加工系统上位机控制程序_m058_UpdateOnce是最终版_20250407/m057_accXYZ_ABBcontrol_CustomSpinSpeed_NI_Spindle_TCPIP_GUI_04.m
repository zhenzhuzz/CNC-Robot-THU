...
function m057_accXYZ_ABBcontrol_CustomSpinSpeed_NI_Spindle_TCPIP_GUI_04()
    %% 初始化参数
    ...
    function [reportData, reportStr] = decisionReport(t_chunk, acc_chunk, currentSpin, threshold)
        ...
        if isChatter
            ...
            if isSuppressActive
                ...
                % 限制转速的最大值为 15000 rpm
                newSpin = min(newSpin, 13000);  % 将转速限制为最大 15000 rpm

                % 调用统一的 updateTargetSpeed 函数更新转速
                updateTargetSpeed(newSpin);  % 传入新的转速值进行更新
            end
        else
    ...
    function prependSpindleDisplay(msg)
        ...
        if isRecordingMessages
            spindleMessages = [spindleMessages; {msg}];  % 将消息添加到主轴日志
        end
    end
