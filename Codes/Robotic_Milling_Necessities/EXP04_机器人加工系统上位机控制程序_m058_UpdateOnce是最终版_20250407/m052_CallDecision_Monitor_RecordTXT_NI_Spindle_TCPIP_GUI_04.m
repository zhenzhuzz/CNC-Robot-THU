% 
% m052_CallDecision_Monitor_RecordTXT_NI_Spindle_TCPIP_GUI_04()
% ### 修改说明（类比解读）
% 
% 1. **窗体标题与位置**  
%     将主窗体的创建修改为
...
%     ```
% 
%     就像更换仪表盘上的显示屏标题和尺寸，使其更符合需求。
% 
% 2. **新增阈值输入**  
%     阈值由初始化参数直接设置，不再显示在界面上。监测模式下调用决策函数时，会传入该阈值参数。就好比在测量仪器上新增了一个“灵敏度阈值”调整按钮。
% 
% 3. **缩短主轴和加速度计面板高度**  
%     通过将主界面第一行高度设为0.8x，使得控制面板（主轴和加速度计的部分）比原来更矮，更紧凑。
% 
% 4. **去除采样率自动调整函数**  
%     因为NI采集卡会自动调整采样率，所以直接取用户输入的采样率（经过四舍五入）即可，不再调用adjustedRate函数。
% 
% 5. **调用决策函数时传入阈值**  
%     在加速度计回调中，如果监测开启，则调用
% 
%     ```matlab
%     m060_Decision_fft_filter_report(t_chunk, acc_chunk, currentSpin, threshold);
%     ```
% 
%     这样可以避免因缺少阈值输入而报错。

function m052_CallDecision_Monitor_RecordTXT_NI_Spindle_TCPIP_GUI_04()
    %% 初始化参数
    % 主轴（Spindle）相关参数
    spindleIP = "127.0.0.1";
    spindlePort = 2000;
    ...
    % 修改3：将主界面第一行高度设置为固定像素（使得主轴和加速度计面板较矮）
    ...
    lblSamplingRate.Layout.Row = 2; lblSamplingRate.Layout.Column = 1;
    ...
    txtSamplingRate.Layout.Row = 2; txtSamplingRate.Layout.Column = 2;


    %% 第二行：记录/监测按钮（横跨整个界面）
    % 修改2：新增一个监测按钮，与记录按钮并排
    ...
    function accelPowerOn(~, ~)
        ...
        try
            ...
            % 采样率直接取文本框的值（NI采集卡自动调整）
            newRate = round(txtSamplingRate.Value);  % 修改：不再调用adjustedRate
    ...
    function accelDAQCallback(src, evt)
        ...
        if ~isempty(timestamps) && ~isempty(data)
            ...
            % 新增：如果监测开启，则调用决策函数，同时传入阈值参数
            if isMonitoring
                % 直接使用当前回调的 timestamps 与 data 作为一个数据块
                reportStr = decisionReport(timestamps, data, currentSpindleSpeed, threshold);
                prependAccelDisplay(reportStr);
            end

            if isRecording
                % 若加速度计上电，则使用其时间戳；若未上电，则记录为NaN（主轴单独上电时记录过）
