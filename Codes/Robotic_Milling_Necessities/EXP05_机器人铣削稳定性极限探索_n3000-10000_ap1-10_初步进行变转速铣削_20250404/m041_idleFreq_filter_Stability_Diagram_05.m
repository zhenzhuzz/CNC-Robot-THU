...
tol = 5;     % Tolerance for harmonic matching
...
for i = 1:nRows
    ...
    % 步骤 5：应用频率滤波
    % Get the frequencies from the idle dataSheet for n = 9000 (no ap required)
    ...
    frequencies = [freq1, freq2, freq3, freq4, freq5];  % All frequencies to filter out
    
    % Apply the frequency filter by removing frequencies within ±tol
    % 调用函数滤波 FFT 幅度
    for k = 1:length(frequencies)
        freq_mask = abs(f - frequencies(k)) < tol;  % Mask for frequencies within tolerance
        X_f(freq_mask) = 0;  % Remove corresponding FFT magnitudes by setting them to 0
    end
    
    % Step 6: Process FFT and find top frequencies
    % 步骤 6：处理 FFT 幅值并查找前 N 个频率
    [topFreq, topAmp, matchResult] = f050_processFFTMagnitude_findTopFreq_filterfsp_findfchatter_05(f, X_f, topN, spinFreq, max(f), tol);

    % Step 7: Check if the top frequency matches spindle harmonics
    % 步骤 7：检查 top1 频率是否与主轴谐波匹配
    if matchResult(1)
        ...
        % Plot 'p' marker for mismatch
        % 如果不匹配，使用 'p' 标记
        marker = 'p';
        markerColor = 'r';  % Red for unstable (does not match spindle harmonics)
        stabilityResults(i) = 1;  % Unstable (does not match spindle harmonics)
        disp('Result: Unstable (does not match spindle harmonics)');
    end

    % Plot the marker in the stability map
    % 在稳定性图上绘制标记
    hold on;
    plot(n, ap, marker, 'MarkerEdgeColor', markerColor, 'MarkerFaceColor', 'none', 'MarkerSize', 8, 'LineWidth', 1.3);
end
