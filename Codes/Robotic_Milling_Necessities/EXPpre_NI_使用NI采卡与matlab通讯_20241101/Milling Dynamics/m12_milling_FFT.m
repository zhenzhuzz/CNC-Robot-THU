...
% including spindle speed tracking and FFT of x displacement.
...
while revs_completed < rev
    ...
    % Display current mode and omega for debugging
    % fprintf('Current Mode: %d | Current Omega: %.2f rpm\n', mode, omega);
    ...
    % Assign current omega to all time points in this revolution
    current_omega_array = repmat(omega, 1, length(time_segment));
    all_omega = [all_omega, current_omega_array];
    ...
    grid on;
    set(gca,'FontSize', 12);
    % axis([0 max(all_time) min(omega0*(1 - RVA)) max(omega0*(1 + RVA))]);
