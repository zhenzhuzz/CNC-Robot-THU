%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% m058_UpdateOnce_accXYZ_ABBcontrol_NI_Spindle_TCPIP_GUI_09.m
% Author：Zhen Zhu
% Source Code on GitHub：https://github.com/zhenzhuzz
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

function m058_UpdateOnce_accXYZ_ABBcontrol_NI_Spindle_TCPIP_GUI_09()
    %% 初始化参数
    ...
    % --- 主轴（Spindle）相关参数 ---
    spindleIP = "192.168.0.1";                % 主轴的 IP 地址
    ...
    function recordButtonCallback(src, ~)
        ...
        if ~isRecording
            ...
            src.Text = '■';
            recordStartTime = tic;
            prependSpindleDisplay('开始记录spindle数据...');
            prependAccelDisplay('开始记录accel数据...');
        else
            % 停止记录数据
            isRecording = false;
            ...
            if ~isempty(spindleMessages) || ~isempty(accelMessages) || ~isempty(robotMessages)
                ...
                try
                    ...
                    spindleMessages = {};  % spindle 消息列表重置为空 cell
                    accelMessages   = {};  % accel 消息列表重置为空 cell
                    robotMessages   = {};  % robot 消息列表重置为空 cell
                catch
    ...
    function monitorButtonCallback(src, ~)
        ...
        if ~isMonitoring
            isMonitoring = true;
            src.Text = '停止监测';
            prependAccelDisplay('监测已开启acc');
            prependSpindleDisplay('监测已开启spindle');
        else
            isMonitoring = false;
            src.Text = '监测开启';
            prependAccelDisplay('监测已停止acc');
            prependSpindleDisplay('监测已停止spindle');
        end
    end
    ...
    function suppressButtonCallback(src, ~)
        if isSuppressActive
            ...
            prependAccelDisplay('抑制关闭acc，主轴转速停止调整');  % 显示抑制关闭消息
            prependSpindleDisplay('抑制关闭spindle，主轴转速停止调整');  % 显示抑制关闭消息
        else
            isSuppressActive = true;
            src.Text = '抑制关闭';  % 修改按钮文字为"抑制关闭"
            prependAccelDisplay('抑制开启acc，主轴转速开始调整');  % 显示抑制开启消息
            prependSpindleDisplay('抑制开启spindle，主轴转速开始调整');  % 显示抑制开启消息
