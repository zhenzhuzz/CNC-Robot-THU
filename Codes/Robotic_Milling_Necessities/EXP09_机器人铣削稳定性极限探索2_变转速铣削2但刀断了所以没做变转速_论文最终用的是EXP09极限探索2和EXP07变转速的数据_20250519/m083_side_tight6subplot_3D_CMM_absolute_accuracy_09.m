...
%% 加载数据 / Load data
load('YerrorBySpeed.mat', 'YerrorBySpeed');  % 从 MAT 文件加载结构体 / load struct
...
for g = 1:numel(spindleGroups)
    ...
    for i = 1:numel(rpms)
        ...
        Zm = YerrorBySpeed.(sprintf('n%d', rpm));
        ...
        ax.YTick      = 1:2:size(Zm,1);                    
        ax.YTickLabel = 3:2:10;                   
    ...
    % close;  % 关闭当前图形 / close this figure
end
