...
file  = 'cmm_09/4000_side.txt';            % 数据文件名 / data file name
...
while ~feof(fid)
    ...
    if startsWith(strtrim(line), 'PT')        % 找到以 PT 开头的行 / lines starting with "PT"
        for k = 1:10
            subline = fgetl(fid);
            if startsWith(strtrim(subline), 'Y')  % 找到以 Z 开头的行 / lines starting with "Z"
...
for i = 1:5
    ...
    f030_optimizeFig_Paper_08( ...
        ...
        '\Delta{\it a}_e (mm)', ...                        % y 轴标签 / y-label
        titleText, ...                             % 子图标题 / title
        [1, 20], ...                                % x 轴范围 / x-limits
        [-0.1, 0.65], ...                             % y 轴范围 / y-limits
